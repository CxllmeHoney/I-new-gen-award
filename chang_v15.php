...
<html lang="th">
  <head>
    ...
    <style>
      * {
      ...
      .input-group {
        display: flex;
        gap: 5px; /* ลดระยะห่างระหว่างช่องกรอกกับปุ่ม */
        margin-bottom: 12px;
      }
      ...
      .location-input {
        ...
        border-radius: 4px;
        font-size: 16px; /* เพิ่มขนาดตัวอักษรเป็น 16px */
        -webkit-text-size-adjust: 100%; /* ป้องกันการปรับขนาดตัวอักษรอัตโนมัติ */
  ...
  <body>
    ...
    <script>
      ...
      // แก้ไขฟังก์ชัน setupRouteSearch
      async function setupRouteSearch() {
        document
          ...
          .addEventListener("click", async () => {
            ...
            try {
              await calculateAndDisplayRoute(originInput, destinationInput);
              // เพิ่มการพับ sidebar หลังจากเริ่มค้นหาเส้นทาง
        ...
        try {
          ...
          if (typeof origin === "string") {
            ...
            if (originResult) {
              ...
              throw new Error("ไม่พบตำแหน่งต้นทาง");
            }
          }
          ...
          const result = await new Promise((resolve, reject) => {
            directionsService.route(request, (result, status) => {
              if (status === "OK") {
                ...
                reject(new Error("ไม่สามารถค้นหาเส้นทางได้"));
              }
            });
          });
