...
<html lang="th">
  ...
  <body>
    ...
    <script>
      ...
      let userMarkerTracking = null;
      let previousPosition = null;
      let watchIdTracking = null;
      ...
      userMarkerTracking = new google.maps.Marker({
        position: pos,
        map: map,
        icon: {
          ...
          fillOpacity: 1,
          strokeWeight: 2,
          rotation: heading, // หมุนตาม heading
        },
        title: "ตำแหน่งของคุณ",
      });
    ...
    `;
      ...
      // ฟังก์ชั่นเริ่มต้นการติดตามตำแหน่งผู้ใช้
      function startUserTracking() {
        if (navigator.geolocation) {
          watchIdTracking = navigator.geolocation.watchPosition(
            (position) => {
              const pos = {
                lat: position.coords.latitude,
                lng: position.coords.longitude,
              ...
              // ถ้า Marker ยังไม่มี ให้สร้างขึ้นมา
              if (!userMarkerTracking) {
                userMarkerTracking = new google.maps.Marker({
                  position: pos,
                  map: map,
                  icon: {
                ...
                // อัพเดทตำแหน่งของ Marker
                userMarkerTracking.setPosition(pos);
              }

              // ซูมเข้าที่ตำแหน่งผู้ใช้ถ้ายังไม่ได้ซูม
              if (!userTracking) {
                map.setCenter(pos);
                map.setZoom(18);
                userTracking = true;
              ...
              // ปรับองศาการเคลื่อนที่ถ้ามีข้อมูล heading
              if (position.coords.heading !== null) {
                const heading = position.coords.heading;
                userMarkerTracking.setIcon({
                  url: "https://maps.google.com/mapfiles/ms/icons/blue-dot.png",
                  scaledSize: new google.maps.Size(50, 50),
                  rotation: heading, // หมุน Icon ตาม heading
                });
              } else if (previousPosition) {
                // คำนวณ bearing ถ้าไม่มีข้อมูล heading
                const bearing = calculateBearing(previousPosition, pos);
                userMarkerTracking.setIcon({
                  url: "https://maps.google.com/mapfiles/ms/icons/blue-dot.png",
                  scaledSize: new google.maps.Size(50, 50),
                  rotation: bearing,
            ...
            (error) => {
              ...
              alert("ไม่สามารถติดตามตำแหน่งของคุณได้");
            },
            {
      ...
      // ฟังก์ชั่นหยุดการติดตามตำแหน่งผู้ใช้
      function stopUserTracking() {
        if (watchIdTracking !== null) {
          navigator.geolocation.clearWatch(watchIdTracking);
          watchIdTracking = null;
        }
        if (userMarkerTracking) {
          userMarkerTracking.setMap(null);
          userMarkerTracking = null;
        }
        userTracking = false;
        previousPosition = null;
      }

      // ฟังก์ชั่นคำนวณทิศทางระหว่างสองตำแหน่ง
