...
<html lang="th">
    ...
    <style>
      * {
      ...
      .route-search-panel {
        ...
        width: 100%; /* ให้ความกว้างเต็ม container */
      ...
      .input-wrapper {
        width: calc(100% - 41px); /* 36px ของปุ่ม + 5px ของ gap */
      }

      .location-input {
        width: 100%;
        height: 36px; /* กำหนดความสูงให้เท่ากับปุ่ม */
        padding: 8px;
      ...
      #destination-input {
        width: calc(100% - 41px); /* ให้มีความกว้างเท่ากับช่องต้นทาง */
      }

      .location-button {
        width: 36px; /* ลดขนาดปุ่ม */
        height: 36px;
        padding: 6px;
        background: #4caf50;
        color: white;
  ...
  <body>
    ...
    <script>
      ...
      async function setupRouteSearch() {
        ...
        // เพิ่ม Places Autocomplete สำหรับต้นทาง
        ...
        // เพิ่ม Places Autocomplete สำหรับปลายทาง
      ...
      // Modify the displayNavigationInfo function to include the turn-by-turn option
      function displayNavigationInfo(result) {
        const route = result.routes[0];
        const leg = route.legs[0];

        // Store the navigation result globally
        window.currentNavigationResult = result;
    ...
    <div style="padding: 15px; position: relative;">
      ...
      <button onclick="startTurnByTurnNavigation(window.currentNavigationResult)" style="
        width: 100%;
        padding: 12px;
        background: #4CAF50;
        color: white;
      ...
      ">
        ...
        เริ่มการนำทางแบบเลี้ยวต่อเลี้ยว
      </button>
    </div>
  `;
      ...
      // Modify the existing stopNavigation function
      function stopNavigation() {
        ...
        if (nextStepInfoWindow) {
          nextStepInfoWindow.close();
          nextStepInfoWindow = null;
        }
      ...
      // Add this function to handle turn-by-turn navigation
      function startTurnByTurnNavigation(result) {
        ...
        // Create info window for next step instructions
        nextStepInfoWindow = new google.maps.InfoWindow();
            ...
            (position) => {
              ...
              if (navigationSteps[currentStep]) {
                const nextStepLocation =
                  navigationSteps[currentStep].start_location;
                ...
                // Update next step info window
                const stepContent = `
            <div style="padding: 10px; max-width: 300px; font-family: 'Kanit', sans-serif;">
              <h3 style="margin: 0 0 10px 0;">ขั้นตอนถัดไป</h3>
              <p style="margin: 0;">${navigationSteps[currentStep].instructions}</p>
              <p style="margin: 5px 0 0 0;"><strong>ระยะทาง:</strong> ${navigationSteps[currentStep].distance.text}</p>
            </div>
          `;

                nextStepInfoWindow.setContent(stepContent);
                nextStepInfoWindow.setPosition(userPos);
                nextStepInfoWindow.open(map);

                // If within 20 meters of next step, move to next step
                if (distanceToNextStep < 20) {
                  currentStep++;
                  if (currentStep >= navigationSteps.length) {
                    stopNavigation();
                    alert("คุณมาถึงจุดหมายแล้ว!");
                  }
                }
              }
            },
        ...
        // Add turn-by-turn navigation panel to popup
        const turnByTurnPanel = document.createElement("div");
        turnByTurnPanel.innerHTML = `
    <div style="margin-top: 15px; padding: 15px; background: #f5f5f5; border-radius: 8px;">
      <h4 style="margin: 0 0 10px 0;">การนำทางแบบเลี้ยวต่อเลี้ยว</h4>
      <div id="navigation-steps" style="max-height: 200px; overflow-y: auto;">
        ${navigationSteps
          .map(
            (step, index) => `
          <div class="navigation-step" style="
            padding: 10px;
            margin: 5px 0;
            background: ${index === 0 ? "#4CAF50" : "white"};
  ...
  `;
        ...
        document.querySelector(".popup-content").appendChild(turnByTurnPanel);
      }
